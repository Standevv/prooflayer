...
export function CopyValue({ value, label, href, full = false }: CopyValueProps) {
  ...
  const display = (
    <span className={`min-w-0 font-mono text-[11px] font-medium text-[#d4d7df] ${full ? "break-all leading-5" : "truncate"}`}>
  ...
  return (
    <div className="relative flex min-w-0 items-center gap-1.5">
      {href === undefined ? display : (
        <a
          className="min-w-0 underline decoration-white/20 underline-offset-4 transition-colors hover:decoration-[#36d17c]/70"
          href={href}
          target="_blank"
          rel="noreferrer"
      ...
      <button
        type="button"
        onClick={copy}
        className="surface-transition grid size-6 shrink-0 place-items-center rounded-[4px] text-[#858a97] hover:bg-white/[0.06] hover:text-[#f0f0f4]"
        aria-label={`Copy ${label}`}
        title={`Copy ${label}`}
      >
        {copyState === "copied" ? (
          <svg viewBox="0 0 16 16" className="size-3.5 text-[#36d17c]" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true">
            <path d="m3 8.2 3 3L13 4.5" />
          </svg>
        ) : copyState === "failed" ? (
          <span className="text-xs font-bold text-[#ff6b6b]" aria-hidden="true">!</span>
        ) : (
