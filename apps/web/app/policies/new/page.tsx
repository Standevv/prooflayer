...
export default async function NewPolicyPage({ searchParams }: { searchParams: Promise<{ preset?: string }> }) {
  ...
  return <div className="min-h-screen bg-[#0b0c10]"><Sidebar /><main className="lg:ml-[240px]"><div className="mx-auto max-w-[1240px] px-4 py-4 sm:px-6 lg:px-8 lg:py-6"><header className="command-header relative overflow-hidden rounded-[9px] border border-white/[0.08] px-5 py-7 sm:px-7 sm:py-8"><div className="relative z-10 max-w-3xl"><Link href="/policies" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8f84dd] hover:text-[#c5bcf7]">← Policy Studio</Link><h1 className="mt-4 text-[34px] font-semibold leading-none tracking-[-0.052em] text-[#f7f7fa] sm:text-[44px]">Create Institutional Policy</h1><p className="mt-3 max-w-2xl text-[12px] leading-5 text-[#aeb2bd]">Configure requirements above ProofLayer’s authoritative verification layer. Typed fields only—no expressions or executable code.</p></div></header><div className="mt-4"><PolicyBuilder presetId={preset} /></div><footer className="mt-5 flex flex-col gap-1 border-t border-white/[0.08] py-4 text-[10px] leading-4 text-[#747987] sm:flex-row sm:justify-between"><p>Structured configuration · Deterministic commitment</p><p>MVP / Pre-production · Local persistence</p></footer></div></main></div>;
}
